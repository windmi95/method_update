...
public class 플레이 {
    ...
    public static void main(String[] args) {
        ...
        가방 가방 = new 가방();
        ArrayList 보유한_소모품 = new ArrayList();
        ...
        while (!게임_진행.게임_종료) {
            ...
            if (메인_메뉴_선택 == 1) {
                ...
                if (운동_메뉴_선택 == 1) {
                    ...
                    int 사용할_소모품_선택 = 가방.아이템_사용(scanner.nextInt());
                    if (가방.보유한_bcaa_개수 > 1) {
                        보유한_소모품.add(0, 가방.보유한_bcaa_개수);
                    } else if (가방.보유한_탄수화물_보충제_개수 > 1) {
                        보유한_소모품.add(1, 가방.보유한_bcaa_개수);
                    } else if (가방.보유한_프로틴_개수 > 1) {
                        보유한_소모품.add(2, 가방.보유한_프로틴_개수);
                    } else if (가방.보유한_크레아틴_개수 >1) {
                        보유한_소모품.add(3, 가방.보유한_크레아틴_개수);
                    } else {
                        System.out.println("아이템이 부족하여 사용하실 수 없습니다.");
                    }
                } else if (운동_메뉴_선택 == 3) {
                    int 들고자_하는_바벨_중량선택 = scanner.nextInt();
                    boolean 중량들수_있는여부  =체육관.선택할_수_있는_무게체크(들고자_하는_바벨_중량선택,게임_캐릭터.레벨, 게임_캐릭터.등급);
